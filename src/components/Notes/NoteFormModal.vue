<template>
  <UModal v-model:open="isOpen" close-icon="i-lucide-x" :title="initialValue ? 'Rename Note' : 'Add New Note'" :ui="{
    title: 'text-text font-medium',
    footer: 'self-end'
  }">
    <slot>
      <UButton title="Add Note" trailing-icon="i-lucide-plus" variant="ghost" />
    </slot>

    <template #body>
      <div class="flex flex-col gap-3">
        <form @submit.prevent="submitNote" class="flex flex-col gap-3" id="note-form">
          <input v-model="title" type="text" placeholder="Note Title (e.g., Daily Standup)" required
